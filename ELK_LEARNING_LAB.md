...
This lab adds a local Elastic stack to the CodeIgniter job board so you can learn search, structured logs, Kibana analysis, and **PHP** APM tracing.
...
- Direct APM Server URL (CLI / debugging inside Docker): `http://apm-server:8200`
...
2. On `/learning/elk`, use **Write logs**, **Handled exception**, **Slow request**, and related PHP demos as needed.
...
2. Open **Services**. You should see **`codeigniter-job-board`** once PHP requests have been traced by the Elastic APM PHP agent.
3. Click the service → **Transactions**, **Errors**, **Spans** to inspect PHP requests and exceptions.

If **Services** is empty: confirm **`lando rebuild -y`** installed the PHP agent (`lando php -m | grep -i elastic`), generate traffic (`/learning/elk`, `/jobs`), and widen the time range.
...
4. Trigger handled and unhandled PHP errors, then compare logs with APM error groups for **`codeigniter-job-board`**.
...
4. Use correlation IDs (`X-Request-Id`) from request logs to pivot to related PHP errors in APM.
